...
class GeoDataset(Dataset):
    def __init__(self, 
        ...
        # Define los demás atributos necesarios para la carga de datos
        ...
        #print(f"self.chips_basedirs: {self.chips_basedirs}")   
    ...
    def __getitem__(self, index):
        ...
        if not isinstance(self.batch_size, int) and self.batch_size.startswith('per_partition'):
         ...
         # Print indexes and batch_chips_basedirs for debugging
        #print(f"__getitem__ indexes: {indexes}")
        #print(f"__getitem__ batch_chips_basedirs: {batch_chips_basedirs}")
    ...
    def __data_generation(self, batch_chips_basedirs):
        ...
        # X : (n_samples, channels, *dim[height, width])
        # Initialization
        batch_size = len(batch_chips_basedirs)
        X = torch.empty((batch_size, 3, 100, 100), dtype=torch.float32)
        labels = torch.zeros((batch_size, 100, 100), dtype=torch.int16)
        partition_proportions = torch.empty((batch_size, self.number_of_output_classes), dtype=torch.float32)

        # Assemble data in a batch
        for i, chip_id in enumerate(batch_chips_basedirs):
            chip_mean, label, p = self.load_chip(chip_id)
            X[i] = torch.from_numpy(chip_mean / 256).permute(2, 0, 1)  # Cambiar el orden de los ejes para PyTorch (C, H, W)
            labels[i] = torch.from_numpy(label)
            partition_proportions[i] = torch.from_numpy(p)
    ...
    # Generate de DataLoader class Pytorch
    np.random.seed(10)
    ...
    def split(self, 
        ...
        train_dataset = GeoDataset(basedir=basedir, 
                 chips_basedirs=tr, 
        ...
        test_dataset = GeoDataset(basedir=basedir, 
                 chips_basedirs=ts, 
        ...
        val_dataset = GeoDataset(basedir=basedir, 
                  chips_basedirs=val, 
        ...
        test_loader = DataLoader(test_dataset, batch_size=None, shuffle=True, num_workers=num_workers)
        val_loader = DataLoader(val_dataset, batch_size=None, shuffle=True, num_workers=num_workers)
             
        return train_loader, test_loader, val_loader   


    def split_per_partition(self, 
                            basedir = None, 
                            partitions_id = None, 
                            cache_size = 1000, 
                            max_chips = None,
        ...
        # create dataloader for each split
        train_dataset = GeoDataset(basedir=datadir, 
        ...
        test_dataset = GeoDataset(basedir=datadir, 
        ...
        val_dataset = GeoDataset(basedir=datadir, 
    ...
    # Retorna la distribución de clases del conjunto de datos
    ...
    # Grafica la distribución de clases del conjunto de datos.
    ...
    # Grafica un número de imágenes de los chips
    ...
    # Grafica una muestra aleatoria de entradas y etiquetas de una tanda de datos
    def plot_sample_batch(self, batch_idx=None, return_fig = False):
        ...
        for ax, ti in subplots(range(n*n_rows), n_cols=n, figsize=(n*3.5, n_rows*3.5)): # figsize=(n*3.5, n_rows*3.5)
            i = ti % n
            row = ti // n
